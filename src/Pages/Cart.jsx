...
const Cart = () => {
    const { cart, incrementQuantity, decrementQuantity, removeFromCart, total, checkout } = useCart();
    const { token } = useUser();
              ...
              {cart.map((item) => (
                <li key={item.id} className="cart-item list-group-item d-flex justify-content-between align-items-center">
                  <div className="cart-item-details d-flex align-items-center">
                    <img src={item.image || item.img || 'https://via.placeholder.com/150'} alt={item.name} className="cart-item-image" style={{ width: '80px', height: '80px', objectFit: 'cover' }} />
